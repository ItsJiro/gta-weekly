...
async function scrapeShowrooms() {
	...
	try {
		...
		$("h2.section-title, h2, h3").each((_, el) => {
			...
			const hasGtaPlusSpan =
				$h2.find('span[id*="gta-monthly-benefits"], span[id*="gta-plus"]')
					.length > 0;

			if (text.includes("GTA+ MONTHLY BENEFITS") || hasGtaPlusSpan) {
				console.log(
					"✂️ Found GTA+ section in DOM. Purging section and all trailing elements...",
				);
				$h2.nextAll().remove();
		...
		const $showroomsSection = $("#showrooms-test-rides").closest(
			".field-entry",
		);
		...
		$("li.gta-bonuses.item-scale").each((_, itemEl) => {
			...
			const $titleLink = $item
				.find("h3.contentheading a, .contentheading a")
				.first();
			...
			const discountPercentage =
				$item.find("div.discounted-price span.badge.new").text().trim() || null;
		...
		console.log(
			`Inserting ${rowsToInsert.length} fresh items into Supabase...`,
		);
